            ...
            return (
              <li key={project.id} className="py-12">
                <div className="max-w-reading">
                  {real(project.category) && <p className="eyebrow">{real(project.category)}</p>}
                  <h2 className="mt-2 font-serif text-3xl tracking-tight">{project.title}</h2>
                  {real(project.short_description) && (
                    <p className="mt-3 text-lg leading-relaxed text-ink/70">{real(project.short_description)}</p>
                  )}

                  {real(project.problem) && (
                    <div className="mt-7">
                      <h3 className="eyebrow mb-2">Problem</h3>
                      <Prose text={real(project.problem)} />
                    </div>
                  )}
                  ...
                  {real(project.solution) && (
                    <div className="mt-6">
                      ...
                      <Prose text={real(project.solution)} />
                    </div>
                  )}
                  ...
                  {real(project.role) && (
                    <p className="mt-6 text-sm text-muted">
                      <span className="text-ink">My role:</span> {real(project.role)}
                    </p>
                  )}
